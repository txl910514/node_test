...
//find 数组实例的find方法，用于找出第一个符合条件的数组成员。它的参数是一个回调函数，所有数组成员依次执行该回调函数，直到找出第一个返回值为true的成
// 员，然后返回该成员。如果没有符合条件的成员，则返回undefined。

var findArr = array.find(function(val,key,list){
  return val>1;
});
/*console.log(findArr);*/
...
var findIndexArr = array.findIndex(function(val,key,list){
  return val>5;
});
